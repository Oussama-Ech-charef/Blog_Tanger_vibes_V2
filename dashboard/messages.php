...
if (isset($_POST['delete']) && is_numeric($_POST['delete'])) {
    if (validate_csrf_token($_POST['csrf_token'] ?? '')) {
        try {
            ...
            $message = 'Message deleted.'; $message_type = 'success';
...
<div class="card">
    <div class="card_header">
        <h2>Message from <?=htmlspecialchars($view_message['full_name'])?></h2>
        <a href="messages.php<?=!empty($search)?'?q='.urlencode($search):''?>" class="btn btn_secondary btn_sm"><i class="fa-solid fa-arrow-left" aria-hidden="true"></i> Back</a>
    </div>
    <div class="card_body">
        <div style="display:grid;gap:20px;">
            ...
            <div style="display:flex;gap:12px;padding-top:12px;border-top:1px solid var(--db-card-border);">
                ...
                <form method="POST" action="messages.php" class="inline_form">
                    ...
                    <button type="submit" class="btn btn_danger" onclick="return confirm('Delete?')"><i class="fa-solid fa-trash"></i> Delete</button>
                </form>
            </div>
        </div>
    </div>
...
<div class="filters_bar">
    <div class="search_input">
        ...
        <form method="GET" id="sf"><input type="text" name="q" placeholder="Search messages..." value="<?=htmlspecialchars($search)?>" onchange="document.getElementById('sf').submit()"></form>
    </div>
    <span style="font-size:14px;color:var(--db-text-secondary);font-weight:500;"><?=$total_records?> message(s)</span>
</div>

<div class="card">
    <div class="card_header"><h2>All Messages</h2></div>
    ...
    <?php if (!empty($search)): ?><input type="hidden" name="q" value="<?=htmlspecialchars($search)?>"><?php endif; ?>
    <button type="submit" class="btn_small btn_danger" onclick="return confirm('Delete?')" aria-label="Delete message"><i class="fa-solid fa-trash"></i></button>
</form></div></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="6"><div class="empty_state"><i class="fa-solid fa-envelope-open-text"></i><h3>No messages</h3><p><?=!empty($search)?'No matches.':'No messages yet.'?></p></div></td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
